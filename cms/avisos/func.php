	...
	else
	{
		...
		if ($dados)
		{
			...
			if($numero!=0)
			{
				...
				{
					if($linha['dataNascimento']!="0000-00-00"){
						$dia=$linha['dia'];
						$mes=devolveMes($linha['mes']);
						$dataAniversario = $dia." de ".$mes;
					}
					else{
						$dataAniversario = "Data de Nascimento inválida";
					}
					print("
						<tr>
							<td><a href='index.php?modulo=clientes&acao=clienteinfo&tipo=detalhes&id=$linha[id]' title='Ver detalhes' class='linkNormal'>$dataAniversario</a></td>
...
function devolveListaImoveisExclusivos($index, $mark, $where, $post)
{
	...
	{
		$cont=0;
		$comandoSql = "SELECT id, referencia, idCliente, imagem, DATE_FORMAT(dataTerminoExclusividade,'%d/%m/%Y') as dataTerminoExclusividade, DATE_FORMAT(dataAviso,'%d/%m/%Y') as dataAviso FROM imoveis $where LIMIT $index,25";
		...
		if ($dados)
		{
			...
			if($numero!=0)
			{
				...
				<table width='100%' id='gradient-style'>
					<tr>
						<th align='left' width='20%'>Data Término Exclusividade</th>
						<th align='left' width='20%'>Data Aviso</th>
				...
				while ($linha = mysql_fetch_array($dados))
				{
					...
					if($linha['dataTerminoExclusividade']!="00/00/0000")
						$dataTermino=$linha['dataTerminoExclusividade'];
					else
						$dataTermino="Data inválida";
					if($linha['dataAviso']!="00/00/0000")
						$dataAviso=$linha['dataAviso'];
					else
						$dataAviso="Data inválida";

					print("
						<tr>
							<td><a href='index.php?modulo=imoveis&acao=imovelinfo&tipo=detalhes&id=$linha[id]' title='Ver detalhes' class='linkNormal'>$dataTermino</a></td>
							<td><a href='index.php?modulo=imoveis&acao=imovelinfo&tipo=detalhes&id=$linha[id]' title='Ver detalhes' class='linkNormal'>$dataAviso</a></td>
