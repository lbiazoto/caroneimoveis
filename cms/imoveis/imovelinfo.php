...
if($modulo!="" && devolveUsuarioLogado() && $_SESSION['imoveis'])
{
	...
	if($modo == "alterarFotoDestaque")
	{
		$retorno=alterarFotoDestaque($destaque, $idImagem);
		if($retorno)
			$msg = "Destaque alterado com sucesso";
		else
			$msg = "Ocorreu uma falha durante a atualização do destaque!";
	}
	...
	if($modo=="adicionarFoto"){
		...
		if(!empty($imagem)){
			for($i=0; $i<count($imagem); $i++){
				$imagem_real	=	$HTTP_POST_FILES['imagem']['name'][$i];
				$imagem_temp	=	$HTTP_POST_FILES['imagem']['tmp_name'][$i];

				$imgNum = devolveNumUpload() .".".end(explode('.', $_FILES['imagem']['name'][$i]));
				$img="imagem".$imgNum;
				$img = strtolower($img);

				$imagem_real = $img;

				move_uploaded_file($imagem_temp, "../imoveis/".$imagem_real);

				imageResize($imagem_real, "../imoveis", "p_", "120", "120");
				imageResize($imagem_real, "../imoveis", "m_", "230", "230");
				imageResize($imagem_real, "../imoveis", "g_", "800", "600");

				$retorno=adicionarFotoImovel($imagem_real, $linha['id']);
				unlink("../imoveis/".$imagem_real);
			}
		}
		/*
		$img[1]="";
		...
		$numimg=1;
		while($numimg<=5){
			$campoimagem ="imagem$numimg";
			if($_FILES[$campoimagem]['tmp_name']!=""){
				$imagem = uploadImagem("$campoimagem", "../imoveis", 0, "");
		...
		$retorno=adicionarFotoImovel($img[1], $img[2], $img[3], $img[4], $img[5], $linha['id']);
		*/
		if($retorno)
			$msg = "Imagens cadastradas com sucesso";
		else
			$msg = "Ocorreu uma falha durante o cadastro das imagens!";
	}
	if($modo=="editarFoto")
	{
		if($imagem!="")
	...
	if($modo=="editar")
	{
		if($tipoImovel!="" && $referencia!="" && $idCliente!="" && $idCorretor!="" && $idCategoria!="" && $idSubCategoria!="" && $valor!="" && $estado!="" && $opcaoCategoria!="" && $listaSubCategoria!="" && $listaSubCategoria2!=""){
			...
			$msg="Ocorreu uma falha durante a atualização do Imóvel!<br>Os campos as seguir, são de preenchimento obrigatório: <b>$campos</b>!";
		}
	}
	...
	if($tipo=="" || $tipo=="detalhes")
	{
		...
		if($linha[exclusivo])
		{
			...
			$exclusivo="Sim / Data de Término: $dataTermino, restam $dias dias / Data de Aviso: $dataAviso";
		}
		else
		{
			$exclusivo="Não";
		...
		$nomeCorretor=devolveInfoCampo("nome", "corretor", $linha['idCorretor']);
		...
		$videoVideo= "
			<script type=\"text/javascript\">
				...
				'width','300',
				'height','250',
				'movie','http://www.youtube.com/v/$codVideo',
		...
		print("
			...
			<table width='100%' align='center' id='gradient-style-detalhes'>
				...
				<tr>
					<td align='left'>&nbsp;$cliente[nome] <a href='index.php?modulo=clientes&acao=clienteinfo&tipo=detalhes&id=$cliente[id]'>Clique aqui para visualizar dados deste cliente</a></td>
					<td align='left'>&nbsp;$tipoCliente</td>
				</tr>
				<tr>
					<th align='left' width='50%'>Tipo do Imóvel</th>
					<th align='left' width='50%'>Referência | Imagem</th>
				</tr>
				<tr>
					<td align='left'>&nbsp;$tipoImovel</td>
					<td align='left'>&nbsp;$linha[referencia] <b>|</b> $verimagem</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<th align='left'>Destaque/Exclusivo</th>
				</tr>
				<tr>
					<td align='left'>$nomeCorretor</td>
					<td align='left'>&nbsp;$destaque&nbsp;/&nbsp;$exclusivo</td>
				</tr>
				<tr>
				...
				<tr>
					<th align='left' colspan='2'>Endereço (Localização no Mapa)</th>
				</tr>
				<tr>
					<td align='left' colspan='2'>&nbsp;$linha[endereco] ($linha[localizacao_mapa])</td>
				</tr>
				<tr>
					<th align='left' colspan='2'>Vídeo</th>
				</tr>
				<tr>
					<td align='left' colspan='2'>&nbsp;$videoVideo</td>
				</tr>
				<tr>
			...
			<form name="formCadastro" id="formCadastro" action="index.php?modulo=imoveis&acao=imovelinfo&tipo=galeria&tipo2=fotos&modo=adicionarFoto&id=<?php print("$id");?>" method="post" enctype="multipart/form-data">
			  	<fieldset>
					...
					<table align="center" border="0" cellspacing="3" cellpadding="3">
						<tr>
							<th>Images:</th>
							<td><input type="file" name="imagem[]" id="imagem" multiple="multiple" /></td>
						</tr>
						<tr>
							<td colspan="2">&nbsp;</td>
						</tr>
				...
				$(document).ready(function() {
					$('#ordemPag').sortable({
						...
						update: function(event, ui){
							var ordem = $(this).sortable('serialize') + '&action=alterarOrdem';

							$.post('imoveis/ordemDB.php', ordem);
						}
					});
				});
			</script>
	...
	elseif($tipo=="editar")
	{
		...
		<form name="formCadastro" id="formCadastro" action="index.php?modulo=imoveis&acao=imovelinfo&modo=editar&id=<?php print("$id");?>" method="post" enctype="multipart/form-data">
		  	<table align="center" width="500" border="0" cellspacing="3" cellpadding="3">
				...
				<tr>
			  		...
			  		<td align="left">
						<select id="tipoImovel" name="tipoImovel" style="width:95%;">
							<option value="0" <?php if($linha['tipo']==0) print("selected");?>>Venda</option>
							<option value="1" <?php if($linha['tipo']==1) print("selected");?>>Loca&ccedil;&atilde;o</option>
						</select> *
					</td>
		 		</tr>
				<tr>
		 		...
		 		<tr>
			  		...
			  		<td align="left">
						<select id="idCorretor" name="idCorretor" style="width:95%;">
							<?php montaSelect("corretor", $linha[idCorretor]); ?>
						</select> *
					</td>
		 		</tr>
		 		<tr>
				...
				<tr>
					<th align="left">V&iacute;deo:</th>
					<td align="left"><input type="text" name="video" value="<?php print($linha['video']);?>" id="video"></td>
				</tr>
				<tr>
					<th align="left" valign="top">V&iacute;deo atual:</th>
					<td align="left"><?php
						$codVideo = end(explode("?v=",$linha['video']));
						$codVideo = explode("&",$codVideo);
						$codVideo = $codVideo[0];
				...
				<tr>
					...
					<td align="left">
                        <input type="text" name="localizacao_mapa" id="localizacao_mapa" value="<?php print($linha['localizacao_mapa']); ?>" maxlength="150">
				...
				<tr>
					<th align="left">&nbsp;</th>
					<td align="left">
                        <label><input type="checkbox" id="marcador" name="marcador" value="1" <?php if($linha[marcador]=="0") print("checked"); ?>> <b>Marque esta op&ccedil;&atilde;o para remover o marcador de localiza&ccedil;&atilde;o.</b></label>
                    </td>
				</tr>
				<tr>
				...
				<tr valign="baseline">
					<td align="center" colspan="2">
						<input name="cadastrar" type="submit" id="form_submit" value="Salvar" onclick="tinyMCE.triggerSave(); return(campoObrigatorio(referencia) && campoObrigatorio(idCliente) && campoObrigatorio(idCategoria) && campoObrigatorio(idSubCategoria) && campoObrigatorio(valor) && campoObrigatorio(opcaoCategoria) && campoObrigatorio(listaSubCategoria) && campoObrigatorio(listaSubCategoria2));">
					</td>
				</tr>
			</table>
		</form>
