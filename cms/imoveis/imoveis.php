...
if($modulo!="" && devolveUsuarioLogado() && $_SESSION['imoveis'])
{
	...
	if($modo=="cadastrar")
	{
		if($tipoImovel!="" && $referencia!="" && $idCliente!="" && $idCorretor!="" && $idCategoria!="" && $idSubCategoria!="" && $valor!="" && $imagem!="" && $estado!="" && $opcaoCategoria!="" && $listaSubCategoria!="" && $listaSubCategoria2!=""){
			$campoimagem ="imagem";
			...
			$retorno=cadastraImovel($tipoImovel, $referencia, $idCliente, $idCategoria, $idSubCategoria, $valor, $video, $imagem, $estado, $opcaoCategoria, $listaSubCategoria, $listaSubCategoria2, $endereco, $localizacao_mapa, $marcador, $descricaoCurta, $descricaoCompleta, $maisInformacoes, $destaque, $exclusivo, converteDataToMysql($dataTerminoExclusividade), converteDataToMysql($dataAviso), $composicao, $idCorretor);
			if($retorno==true)
			{
				print("
			...
			if($opcaoCategoria=="")
				$campos .= "Cidade, ";
			if($listaSubCategoria=="")
				$campos .= "Região, ";
			if($listaSubCategoria2=="")
				$campos .= "Bairro, ";

			$campos = substr($campos, 0, -2);
			$msg="Erro ao cadastrar Imóvel!<br>Os campos as seguir, são de preenchimento obrigatório: <b>$campos</b>!";
		}
	}
	...
	if($tipo=="cadastrar")
	{
		...
		<form name="formCadastro" id="formCadastro" action="index.php?modulo=imoveis&acao=imoveis&modo=cadastrar&tipo=cadastrar" method="post" enctype="multipart/form-data">
		  	<table align="center" width="500" border="0" cellspacing="3" cellpadding="3">
				...
				<tr>
			  		<th align="left">Tipo do Im&oacute;vel:</th>
			  		<td align="left">
						<select id="tipoImovel" name="tipoImovel" style="width:95%;">
				...
				<tr>
					...
					<td align="left">CI&nbsp;<input type="text" name="referencia" id="referencia" > *</td>
				</tr>
				<tr>
				...
				<tr>
			  		<th align="left">Corretor:</th>
			  		<td align="left">
						<select id="idCorretor" name="idCorretor" style="width:95%;">
							<?php montaSelect("corretor", $idCorretor); ?>
						</select> *
					</td>
		 		</tr>
		 		<tr>
				...
				<tr>
					<th align="left" valign="top">V&iacute;deo:</th>
					<td align="left"><input type="text" name="video" id="video"></td>
				</tr>
				<tr>
					<th align="left">Estado:</th>
					<td align="left"><?php montaComboEstado($estado, false, false);?> *</td>
				</tr>
				<tr>
				...
				<tr>
					<th align="left">Localiza&ccedil;&atilde;o Mapa:</th>
					<td align="left">
                        <input type="text" name="localizacao_mapa" id="localizacao_mapa" maxlength="150">
                        &nbsp;<a target="_blank" href="http://maps.google.com/maps?f=q&source=s_q&hl=pt-BR&geocode=&q=Br,+SC,+Joinville&sll=-26.312007,-48.843809&sspn=0.006405,0.013078&ie=UTF8&hq=&hnear=Joinville+-+Santa+Catarina,+Brasil&ll=-26.303784,-48.844335&spn=0.012811,0.026157&z=16"><img src="imagens/localizar_no_mapa.png" border="0" /></a>
                    </td>
				</tr>
				<tr>
					<th align="left">&nbsp;</th>
					<td align="left">
                        <label><input type="checkbox" id="marcador" name="marcador" value="1"> <b>Marque esta op&ccedil;&atilde;o para remover o marcador de localiza&ccedil;&atilde;o.</b></label>
                    </td>
				</tr>
				<tr>
				...
				<tr valign="baseline">
					<td align="center" colspan="2">
						<input name="cadastrar" type="submit" id="form_submit" value="Cadastrar" onclick="tinyMCE.triggerSave(); return(campoObrigatorio(referencia) && campoObrigatorio(idCliente) && campoObrigatorio(idCategoria) && campoObrigatorio(idSubCategoria) && campoObrigatorio(valor) && campoObrigatorio(imagem) && campoObrigatorio(opcaoCategoria) && campoObrigatorio(listaSubCategoria) && campoObrigatorio(listaSubCategoria2));">
					</td>
				</tr>
			</table>
		</form>
	...
	elseif($tipo=="listar" || $tipo=="")
	{
		...
		//if($order=="") $referenciaDescSelected="selected";
		...
		if($order=="nomeASC") $nomeAscSelected="selected"; else $nomeAscSelected="";
		if($order=="nomeDESC") $nomeDescSelected="selected"; else $nomeDescSelected="";
		...
		if($statusImovel=="")	$sstatusImovel="selected";	else	$sstatusImovel="";
		if($statusImovel=="0")	$sstatusImovel0="selected";	else	$sstatusImovel0="";
		if($statusImovel=="1")	$sstatusImovel1="selected";	else	$statusImovel1="";

		if($destaqueImovel=="")	$sdestaqueImovel="selected";	else	$sdestaqueImovel="";
		if($destaqueImovel=="0")	$sdestaqueImovel0="selected";	else	$sdestaqueImovel0="";
		if($destaqueImovel=="1")	$sdestaqueImovel1="selected";	else	$sdestaqueImovel1="";

		$selTipoImovel="selTipoImovel$tipoImovel";
		$$selTipoImovel="selected";
		print("
						...
						<fieldset style='width:450px;'>
							...
							<table align = 'center' border = '0' cellpadding='2' cellspacing='2'>
								<tr>
									...
									<td align='left'>
										<select name='order' id='order'>
											...
											<option value='nomeASC' $nomeAscSelected>Nome Cliente Crescente</option>
											<option value='nomeDESC' $nomeDescSelected>Nome Cliente Decrescente</option>
										</select>
									</td>
								</tr>
								<tr>
									...
									");
										montaSelect("corretor", $idCorretor);
									print("
										</select>
									</td>
						 		</tr>
								<tr>
									...
									<td align='left'>
										<select name='statusImovel' id='statusImovel'>
											<option value='' selected>Todos</option>
                                            <option value='0' $sstatusImovel0>Inativos</option>
											<option value='1' $sstatusImovel1>Ativos</option>
										</select>
									</td>
								</tr>
								<tr>
									...
									<td align='left'>
										<select name='destaqueImovel' id='destaqueImovel'>
											<option value='' selected>Todos</option>
                                            <option value='0' $sdestaqueImovel0>Destaques</option>
											<option value='1' $sdestaqueImovel1>Não Destaques</option>
										</select>
									</td>
								</tr>
								<tr>
									<th align='left'>Tipo do Imóvel:</th>
									<td align='left'>
										<select name='tipoImovel' id='tipoImovel'>
											<option value='' selected>Todos</option>
                                            <option value='1' $selTipoImovel1>Locação</option>
											<option value='0' $selTipoImovel0>Venda</option>
								...
								<tr>
							  		<th align='left'>Categoria:</th>
							  		<td align='left'>
										<select id='idCategoria' name='idCategoria' style='width:95%;'>
											<option value=''>Todos</option>
										");
											montaSelect("categorias", $idCategoria);
									print("
										</select>
									</td>
						 		</tr>
									...
									<td align='left'>CI&nbsp;<input type='text' name='referencia' id='referencia' value='$referencia' style='width:90%;'></td>
								</tr>
								<tr>
									<th align='left'>Nome/Conjuge:</th>
									<td align='left'><input type='text' name='nome' id='nome' value='$nome' size='40'></td>
								</tr>

								<tr>
									<th align='left'>Rua:</th>
									<td align='left'><input type='text' name='rua' id='rua' value='$rua' size='25'></td>
								</tr>
								<tr>
									<th align='left'>Bairro:</th>
									<td align='left'>
		...
		if($order=="") $order="dataCadastroDESC";

		$where="";
		if($tipoImovel!=""){
			$where.=" AND imo.tipo = $tipoImovel ";
		}
        if($referencia!=""){
			$where.=" AND imo.referencia LIKE '%$referencia%' ";
		}
		if($idBairro!=""){
			$where.=" AND imo.idBairro =$idBairro ";
		}
		if($valorIni!="")
		{
			$valorFiltroIni=formataValor($valorIni);
			$where.=" AND imo.valor>=$valorFiltroIni ";
		}
		if($valorFim!="")
		{
			$valorFiltroFim=formataValor($valorFim);
			$where.=" AND imo.valor<=$valorFiltroFim ";
		}
		if($exclusivos!="")
		{
			$where.=" AND imo.exclusivo=$exclusivos ";
		}

		if($statusImovel!="")
		{
			$where.=" AND imo.status='$statusImovel' ";
		}

		if($destaqueImovel!="")
		{
			$where.=" AND imo.destaque='$destaqueImovel' ";
		}

		if($rua !="")
		{
			$where.=" AND imo.endereco LIKE '%$rua%' ";
		}
		if($idCategoria !="")
		{
		...
		if($idCorretor !="")
		{
			$where.=" AND imo.idCorretor = '$idCorretor' ";
		}
		if($nome !="")
		{
			$where.=" AND (cli.nome LIKE '%$nome%' OR cli.nomeConjuge LIKE '%$nome%') ";
		}



		if($order=="dataCadastroDESC")	$orderBy="imo.id DESC";
		if($order=="referenciaASC")	$orderBy="imo.referencia ASC";
		if($order=="referenciaDESC")	$orderBy="imo.referencia DESC";
		if($order=="valorASC")	$orderBy="imo.valor ASC";
		if($order=="valorDESC")	$orderBy="imo.valor DESC";
		if($order=="nomeASC")	$orderBy="cli.nome ASC";
		if($order=="nomeDESC")	$orderBy="cli.nome DESC";
		print("
			<table width='100%' cellpadding='2' cellspacing='2'>
				<tr>
					<td align='right'>
						<a href='imoveis/imprimir.php?where=".urlencode($where)."' title='Imprimir' target='_blank'>
							<div class='app_botao' style='margin:0px;'><img src='imagens/printer.png' border='0' class='icone_botao'> Imprimir</div>
						</a>
					</td>
				</tr>
			</table>
			");
		devolveListaImoveis($tipo, $index, $mark, "imo.deletado='0' $where ORDER BY $orderBy", "&idCorretor=$idCorretor&idCategoria=$idCategoria&statusImovel=$statusImovel&order=$order&referencia=$referencia&idBairro=$idBairro&valorIni=$valorIni&valorFim=$valorFim&rua=$rua&nome=$nome", $statusImovel);
	}
}
