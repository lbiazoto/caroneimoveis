...
function cadastraImovel($tipo, $referencia, $idCliente, $idCategoria, $idSubCategoria, $valor, $video, $imagem, $idEstado, $opcaoCategoria, $listaSubCategoria, $listaSubCategoria2, $endereco, $localizacao_mapa, $descricaoCurta, $descricaoCompleta, $maisInformacoes, $destaque, $exclusivo, $dataTerminoExclusividade, $dataAviso, $composicao, $corretor)
{
	...
	{
		$comandoSql = "
			INSERT INTO imoveis (idCorretor, tipo, referencia, idCliente, idCategoria, idSubCategoria, valor, video, imagem, idEstado, idCidade, idRegiao, idBairro, endereco, localizacao_mapa, descricaoCurta, descricaoCompleta, maisInformacoes, destaque, exclusivo, dataTerminoExclusividade, dataAviso, idUsuarioCadastro, dataCadastro)
			VALUES ('$corretor','$tipo','$referencia','$idCliente','$idCategoria','$idSubCategoria','$valor','$video','$imagem','$idEstado','$opcaoCategoria','$listaSubCategoria','$listaSubCategoria2','$endereco','$localizacao_mapa','$descricaoCurta','$descricaoCompleta','$maisInformacoes','$destaque','$exclusivo','$dataTerminoExclusividade','$dataAviso','$_SESSION[idUsuario]',now());
		";
		debug($comandoSql);
...
function editaImovel($idCorretor, $tipo, $referencia, $idCliente, $idCategoria, $idSubCategoria, $valor, $video, $imagem, $idEstado, $idCidade, $idRegiao, $idBairro, $endereco, $localizacao_mapa, $descricaoCurta, $descricaoCompleta, $maisInformacoes, $destaque, $exclusivo, $dataTerminoExclusividade, $dataAviso, $composicao, $codigo)
{
	include("./config.php");
	if (!conectaBancoDados())
		print("<center><strong>Nao foi possível estabelecer conexao com o Banco de Dados!</strong></center>");
...
function devolveListaFotosImovel($idImovel){
	...
	if (!conectaBancoDados())
		...
		if($dados && mysql_num_rows($dados)){
			print("Clique nas imagens e arraste para ordenar as páginas.");
...
function devolveListaImoveisDestaques($tipo, $index, $mark, $where, $post)
{
	...
	else
	{
		...
		if ($dados)
		{
			...
			else
			{
				print("
					<table width='100%' id='gradient-style'>
						...
						<tr>
							<td colspan='4'>
								Nenhum registro de imóveis em destaque encontrado.
							</td>
						</tr>
					</table>
				");
			...
			print("<center><strong>Erro na exibiçao dos imóveis em destaque!</strong></center>");
	}
}
...
function devolveListaImoveisImprimir($where)
{
	...
	{
		$cont=0;
		$comandoSql = "SELECT cli.nome, cli.telefone, cli.telefoneConjuge, cli.telefoneComercial, cli.telefoneResidencial, imo.*, date_format(imo.dataCadastro, '%d/%m/%Y') as dataCadastro FROM imoveis imo, cliente cli WHERE cli.id = imo.idCliente $where	";
		...
		if ($dados)
		{
			...
			if($numero!=0)
			{
				...
				<table width='800' align='center' cellpadding='3'>
					<tr>
						<th colspan='6'>Carone Imóveis<BR>Impressão de Imóveis - Gerado em $dataG<BR><BR></th>
					</tr>
					<tr>
