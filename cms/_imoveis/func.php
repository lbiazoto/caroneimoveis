<?php
function cadastraImovel($tipo, $referencia, $idCliente, $idCategoria, $idSubCategoria, $valor, $imagem, $idEstado, $opcaoCategoria, $listaSubCategoria, $listaSubCategoria2, $endereco, $localizacao_mapa, $descricaoCurta, $descricaoCompleta, $maisInformacoes, $destaque, $exclusivo, $dataTerminoExclusividade, $dataAviso, $composicao)
{
	include("./config.php");
	...
	{
		$comandoSql = "
			INSERT INTO imoveis (tipo, referencia, idCliente, idCategoria, idSubCategoria, valor, imagem, idEstado, idCidade, idRegiao, idBairro, endereco, localizacao_mapa, descricaoCurta, descricaoCompleta, maisInformacoes, destaque, exclusivo, dataTerminoExclusividade, dataAviso, idUsuarioCadastro, dataCadastro)
			VALUES ('$tipo','$referencia','$idCliente','$idCategoria','$idSubCategoria','$valor','$imagem','$idEstado','$opcaoCategoria','$listaSubCategoria','$listaSubCategoria2','$endereco','$localizacao_mapa','$descricaoCurta','$descricaoCompleta','$maisInformacoes','$destaque','$exclusivo','$dataTerminoExclusividade','$dataAviso','$_SESSION[idUsuario]',now());
		";
		debug($comandoSql);
...
function editaImovel($tipo, $referencia, $idCliente, $idCategoria, $idSubCategoria, $valor, $imagem, $idEstado, $idCidade, $idRegiao, $idBairro, $endereco, $localizacao_mapa, $descricaoCurta, $descricaoCompleta, $maisInformacoes, $destaque, $exclusivo, $dataTerminoExclusividade, $dataAviso, $composicao, $codigo)
{
	include("./config.php");
	if (!conectaBancoDados())
		print("<center><strong>Nao foi possível estabelecer conexao com o Banco de Dados!</strong></center>");
	else
	{
		...
		$comandoSql = "
			UPDATE imoveis SET tipo='$tipo', referencia='$referencia', idCliente='$idCliente', idCategoria='$idCategoria', idSubCategoria='$idSubCategoria', valor='$valor' $imagem, idEstado='$idEstado', idCidade='$idCidade', idRegiao='$idRegiao', idBairro='$idBairro',
			endereco='$endereco',localizacao_mapa='$localizacao_mapa',descricaoCurta='$descricaoCurta', descricaoCompleta='$descricaoCompleta', maisInformacoes='$maisInformacoes', destaque='$destaque', exclusivo='$exclusivo' $datas, idUsuarioAtualizacao='$_SESSION[idUsuario]', dataAtualizacao=now() WHERE id=$codigo
		";
		debug($comandoSql);
		$dados = mysql_db_query($bancoDados, $comandoSql);
		if ($dados)
...
function devolveListaImoveis($tipo, $index, $mark, $where, $post)
{
	include("./config.php");
	...
	else
	{
		...
		if ($dados)
		{
			...
			if($numero!=0)
			{
				devolveNavegacao($index, "index.php?modulo=imoveis&acao=imoveis&tipo=listar", $where, $mark, "imoveis", $post);
				print("
			...
			else
			{
				print("
					<table width='100%' id='gradient-style'>
						...
						<tr>
							<td colspan='4'>
								Nenhum registro de imóveis cadastrado.
							</td>
						</tr>
					</table>
				");
		...
		if ($dados && mysql_num_rows($dados)>0)
		{
			print("
				<table width='100%' align='center' id='gradient-style-detalhes'>
					<tr>
						<th align='left' colspan='4'>Composição</th>
					</tr>
					");
					$cont=0;
					while($linha = mysql_fetch_array($dados))
			...
			else
			{
				print("
					<table width='100%' id='gradient-style'>
						...
						<tr>
							<td colspan='5'>
								Nenhum registro de Categoria cadastrado para o módulo Venda.
							</td>
						</tr>
					</table>
				");
...
function adicionarFotoImovel($imagem, $idImovel)
{
	...
	{
		$comandoSql = "INSERT INTO galeriafotos (idImovel, imagem, idUsuarioCadastro, dataCadastro) VALUES ('$idImovel', '$imagem', '$_SESSION[idUsuario]', now())";
		$dados = mysql_db_query($bancoDados, $comandoSql);
		if ($dados)
		{
			$retorno[0]=true;
			$retorno[1]=mysql_insert_id();
			return $retorno;
		}
	}
	return false;
...
function editaFotoImovel($imagem, $codigo)
{
	...
	if (!conectaBancoDados())
		...
		if($imagem) $imagem=" imagem='$imagem', ";
		$comando = "UPDATE galeriafotos SET $imagem idUsuarioAtualizacao='$_SESSION[idUsuario]', dataAtualizacao=now() WHERE id = $codigo";
		$dados = mysql_db_query($bancoDados, $comando);
		if ($dados)	return true;
	}
	return false;
...
function devolveListaFotosImovel($idImovel)
{
	...
	{
		$cont=0;
		$comandoSql = "SELECT *, date_format(dataCadastro, '%d/%m/%Y') as dataCadastro FROM galeriafotos WHERE idImovel=$idImovel AND deletado=0 ORDER BY id DESC";
		debug("<b>$comandoSql</b>");
		$dados = mysql_db_query($bancoDados, $comandoSql);
		if ($dados)
		{
			...
			if($numero>0)
			{
				...
				<table width='100%' id='gradient-style'>
					<tr>
						<th align='left' width='20%'>Data de Cadastro</th>
						<th align='left' width='60%'>Imagem</th>
						<th colspan='3'>&nbsp;</th>
					</tr>
				");
				while ($linha = mysql_fetch_array($dados))
				{
					if($linha[imagem])
					{
						$verimagem = "<a href='../imoveis/m_$linha[imagem]' id='thumb1' title='$linha[nome]' class='highslide' onclick=\"return hs.expand(this)\"><img src='imagens/visualizar.png' border='0' class='icone_botao'> $linha[imagem]</a>";
					}
					else
					{
						$verimagem="&nbsp;";
					}
					$img_status="<img src='imagens/status$linha[destaque].png' border='0'>";
					if($linha[destaque]==1)
					{
					...
					$status="<a href='index.php?modulo=imoveis&acao=imovelinfo&tipo=galeria&tipo2=fotos&modo=alterarDestaque&status=$linha[destaque]&id=$idImovel&idImagem=$linha[id]' title='$title_status'>$img_status</a>";
					print("
						<tr>
							<td title='$linha[dataCadastro]'>$linha[dataCadastro]</td>
							<td width='24'>$verimagem</td>
							<td width='24'>$status</td>
							<td width='24'><a href='index.php?modulo=imoveis&acao=imovelinfo&tipo=galeria&tipo2=editarFoto&id=$idImovel&idImagem=$linha[id]' title='Editar'><img src='imagens/editar.png' border='0'></a></td>
							<td width='24'><a href='index.php?modulo=imoveis&acao=imovelinfo&tipo=galeria&tipo2=fotos&modo=excluirFoto&id=$idImovel&idImagem=$linha[id]' title='Excluir' onclick='return confirmaExclusao();'><img src='imagens/delete.png' border='0'></a></td>
						</tr>
					");
				}
				print("
			...
			else
			{
				print("
					<table width='100%' id='gradient-style'>
						<tr>
							<th align='left' width='20%'>Data de Cadastro</th>
							<th align='left' width='80%'>Imagem</th>
						</tr>
						<tr>
			...
			print("<center><strong>Erro na exibiçao das imagens!</strong></center>");
	}
}
function removeArquivoGaleria($tabela, $codigo)
{
	...
	{
		$comando = "DELETE FROM $tabela WHERE id = $codigo";
		$dados = mysql_db_query($bancoDados, $comando);
		if ($dados)	{
			return true;
		}
