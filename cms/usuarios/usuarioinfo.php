<?php
if($modulo!="" && devolveUsuarioLogado() && $_SESSION['usuarios'] && $id!="")
{
	include("func.php");

	if($tipo=="" || $tipo=="detalhes")
		$titulo = "Usuários - Detalhes do Usuário";
	elseif($tipo=="editar")
		$titulo = "Usuários - Editar Usuário";

	$linha=devolveInfo("usuario", $id);

	if($modo=="atualizar")
	{
		$retorno=atualizaUsuario($id, $nome, $nomeLogin, $nomeSenha, $email, $idModuloInicial, $modulos, $_SESSION['login']);
		if($retorno)
			...
			<table width='100%' cellpadding='2' cellspacing='2'>
				<tr>
					<td width='90%' align='right'>
						<a href='index.php?modulo=usuarios&acao=usuarioinfo&tipo=editar&id=$id' title='Editar Usuário'>
							<div class='app_botao'><img src='imagens/editar.png' border='0' class='icone_botao'> Editar</div>
						</a>
					</td>
					<td align='right'>
						<a href='index.php?modulo=usuarios&acao=usuarioinfo&tipo=&modo=remover&id=$id' onclick='return confirmaExclusao();' title='Excluir Usuário'>
	...
	elseif($tipo=="editar")
	{
		print("
			<table width='100%' cellpadding='2' cellspacing='2'>
				<tr>
					<td align='right'>
						<a href='index.php?modulo=usuarios&acao=usuarioinfo&tipo=&id=$id' title='Ver detalhes do Usuário'>
		...
		$vetorModulos=devolveVetorModulosPermitidos($linha[id]);
		...
		<script language="javascript">
			...
			function validaPermissoesOnSubmit(valor)
			{
				if (valor < 1)
				{
					alert("É necessário que seja atribuída permissão para pelo menos um módulo!");
					return false;
				}
				return true;
			}
			...
			function validaSenhaConfirm(valor, senhaAtual)
			{
				...
				{
					obj_msg.className = "obrigatorio";
					obj_msg.innerHTML = "<div class='imgMsg'><img src='css/images/obrigatorio.png'></div> Confirme a nova senha.";
				}
			}
			function validaSenhaConfirmOnsubmit(id, senhaAtual)
			{
				...
				if(txt != senhaAtual)
				{
					alert("O campo \"Confirmação de Senha\" deve corresponder a nova senha!");
					obj.focus();
					return false;
				}
				return true;
		...
		<form action="index.php?modulo=usuarios&acao=usuarioinfo&modo=atualizar&id=<?php print("$linha[id]"); ?>" id="formEditar" name="formEditar" method="post" enctype="multipart/form-data">
			<fieldset>
		    	...
		    	<table align="center" border="0" cellspacing="3" cellpadding="3">
					...
					<tr>
						...
						<td align="left"><?php montaCheckboxesModulosEdicao("formEditar", $vetorModulos); ?></td>
						<td valign="top">
					...
					<tr>
						<td colspan="3" align="center">
							<input type="submit" id="form_submit" name="Submit" value="Salvar" onclick="
								...
								{
									return(campoObrigatorio(nome) && validaEmailOnSubmit('email') && validaPermissoesOnSubmit(tamanhoVetor));
								}
							">
						</td>
					</tr>
