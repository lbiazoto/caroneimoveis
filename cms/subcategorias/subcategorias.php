...
if($modulo!="" && devolveUsuarioLogado() && $_SESSION['subcategorias'])
{
	...
	if($tipo=="cadastrar")	$tituloH1="Sub Categorias - Cadastrar";
	elseif($tipo=="listar")	$tituloH1="Sub Categorias - Listar";

	if($modo=="cadastrar")
	{
		$retorno=cadastraSubcategoria($idCategoria, $nome, $_SESSION['idUsuario']);
		if($retorno==true)
			$msg="Sub Categoria cadastrado com sucesso!";
		else
			$msg="Erro ao cadastrar Sub Categoria!";
	}
	...
	{
		?>
		<form name="formCadastro" id="formCadastro" action="index.php?modulo=subcategorias&acao=subcategorias&modo=cadastrar&tipo=cadastrar" method="post" enctype="multipart/form-data">
		  	<fieldset>
		    	<legend>Cadastrar</legend>
		    	<table align="center" border="0" cellspacing="3" cellpadding="3">
					<tr id="combo">
						<th align="left">Categoria:</th>
						<td align="left"><?php montaSelectCategorias("idCategoria", $idCategoria); ?></td>
						<td valign="top">
					...
					<tr>
						...
						<td align="left"><input type="text" name="nome" id="nome" maxlength="100" onfocus="validaInputText(this,'msgNome','Nome da Sub Categoria')"  onkeyup="validaInputText(this,'msgNome','Nome da Sub Categoria')"></td>
						<td>
	...
	elseif($tipo=="listar" || $tipo=="")
	{
		...
		<table align='center' width='90%' cellpadding='2' cellspacing='2'>
			<tr>
				<td width='80%' align='center' valign='top'>
					<form action='index.php?modulo=subcategorias&acao=subcategorias&tipo=listar&modo=pesquisa' method='post'>
		...
		devolveListaSubcategorias($tipo, $index, $mark, "WHERE deletado='0' AND id!='-1' $where ORDER BY $orderBy", $order, $titulo, "&modo=pesquisa&titulo=$titulo&order=$order");
	}
}
else
{
